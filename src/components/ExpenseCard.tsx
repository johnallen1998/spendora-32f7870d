  ...
  return (
    <div className="bg-white dark:bg-gray-900 rounded-lg p-4 mb-3 shadow-sm flex justify-between items-center">
      <div className="flex items-center">
        <div className={`w-12 h-12 rounded-full flex items-center justify-center mr-4 ${getCategoryBgColor(expense.category)}`}>
          <CategoryIcon category={expense.category} />
        </div>
        <div>
          <h3 className="font-semibold">{expense.title}</h3>
          <p className="text-gray-500 dark:text-gray-400 text-sm">{expense.category.charAt(0).toUpperCase() + expense.category.slice(1)}</p>
        </div>
      </div>
